...
from .IocainePowder_Cython import match_single, match_both, StatsCython, do_history, do_predict, PredictCython, scan_predict 
...
class IocaineBot:
    ...
    # Function that calculates the Post-Game Statistics.
    def calculate_stats(self):
        rounds = len(self.my_history)
        # for no rounds played, return empty stats
        if rounds == 0:
            return {
                "rounds": 0, "user_wins": 0, "bot_wins": 0, "ties": 0,
                "user_rock": 0, "user_paper": 0, "user_scissors": 0
            } 
        
        stats = {"rounds": rounds, "user_wins": 0, "bot_wins": 0, "ties": 0}

        # Move counts (0: rock, 1: paper, 2: scissors)
        user_moves = {0: 0, 1: 0, 2: 0}

        for i in range(rounds):
            user_move = self.opp_history[i]
            bot_move = self.my_history[i]
            
            user_moves[user_move] += 1

            # Define a 'tie' condition
            if user_move == bot_move:
                stats["ties"] += 1
             # User wins if their move beats the bot's move
            elif will_beat[bot_move] == user_move:
                stats["user_wins"] += 1
            else:
                stats["bot_wins"] += 1

        stats["user_rock"] = user_moves[0]
        stats["user_paper"] = user_moves[1]
        stats["user_scissors"] = user_moves[2]
